...
def repo_root() -> Path:
    ...
    try:
        cand = p.parents[3]
        if (cand / ".team-os").exists() and (cand / "AGENTS.md").exists():
            return cand
    except Exception:
        pass
...
def add_default_args(ap: argparse.ArgumentParser) -> None:
    ...
    ap.add_argument("--workspace-root", default="", help="override workspace root (default: ~/.teamos/workspace or env)")
...
def resolve_repo_root(args: argparse.Namespace) -> Path:
    if str(getattr(args, "repo_root", "") or "").strip():
        ...
        if not (p / ".team-os").exists():
            raise PipelineError(f"invalid --repo-root (missing .team-os): {p}")
        return p
    return repo_root()
