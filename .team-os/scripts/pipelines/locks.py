...
def acquire_scope_lock(
    ...
    *,
    ...
    rr = (repo_root or Path.cwd()).resolve()
    ws = (workspace_root or Path(os.getenv("TEAMOS_WORKSPACE_ROOT") or (Path.home() / ".teamos" / "workspace"))).expanduser().resolve()
    ...
    # File lock path: teamos -> repo state; project -> workspace project state; fallback -> req_dir/.locks
    lock_dir: Path
    if s == "teamos":
        lock_dir = rr / ".team-os" / "state" / "locks"
        ...
        if req_dir is not None and not lock_dir.exists():
            # For unit tests: allow req_dir-local lock dir when workspace scaffold isn't present.
            lock_dir = (req_dir / ".locks").resolve()
