...
def _read_policy(repo: Path) -> dict[str, Any]:
    ...
    pol_path = repo / ".team-os" / "policies" / "self_improve.yaml"
...
def _scan(repo: Path) -> dict[str, Any]:
    ...
    missing_pipelines = []
    if not (repo / ".team-os" / "scripts" / "pipelines" / "self_improve_daemon.py").exists():
        missing_pipelines.append(".team-os/scripts/pipelines/self_improve_daemon.py")
    if not (repo / ".team-os" / "scripts" / "pipelines" / "task_ship.py").exists():
        missing_pipelines.append(".team-os/scripts/pipelines/task_ship.py")
...
def _proposals_from_scan(scan: dict[str, Any], *, min_n: int) -> list[Proposal]:
    ...
    for p in (scan.get("missing_pipelines") or []):
        if str(p) == ".team-os/scripts/pipelines/task_ship.py":
            ...
            acc = ["新增 pipeline: .team-os/scripts/pipelines/task_ship.py", "CLI: ./teamos task ship 可用并可 dry-run"]
            evidence = [f"missing: {p}"]
            props.append(
                Proposal(
                    key=_proposal_key(title, text, acc),
...
def run_once(
    ...
    *,
    ...
    state["policy_sha256"] = _sha256_text(_read_text(repo / ".team-os" / "policies" / "self_improve.yaml"))
    state["leader"] = leader
    state["last_run"] = {
        "ts": utc_now_iso(),
        "ok": True,
