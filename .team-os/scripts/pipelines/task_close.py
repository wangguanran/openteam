...
def _locate_task(repo: Path, ws_root: Path, *, scope: str, task_id: str) -> tuple[str, str, Path, Path]:
    ...
    # 1) explicit teamos
    if scope in ("", "teamos"):
        led = _find_task_in_dir(repo / ".team-os" / "ledger" / "tasks", task_id)
        if led:
            logs = repo / ".team-os" / "logs" / "tasks" / task_id
...
def main(argv: list[str] | None = None) -> int:
    ...
    tel_schema = repo / ".team-os" / "schemas" / "telemetry_event.schema.json"
    ...
    try:
        sys.path.insert(0, str(repo / ".team-os" / "scripts"))
