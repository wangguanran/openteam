...
def _task_paths(*, repo: Path, ws_root: Path, scope: str, project_id: str) -> tuple[Path, Path]:
    ...
    """
    if scope == "teamos":
        return (repo / ".team-os" / "ledger" / "tasks", repo / ".team-os" / "logs" / "tasks")

    # project scope must live in Workspace (outside repo)
    if (ws_root / "projects").exists() and (ws_root / "projects").resolve() == (repo / "projects").resolve():
        # unlikely; but keep defensive.
        raise PipelineError("workspace_root points inside repo (refusing)")
    if ws_root.exists() and (repo / ".team-os").exists():
...
def _render_log_from_tpl(repo: Path, *, name: str, task_id: str, title: str) -> str:
    tpl = repo / ".team-os" / "templates" / f"task_log_{name}"
    if not tpl.exists():
