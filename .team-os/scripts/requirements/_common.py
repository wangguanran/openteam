...
def repo_root() -> Path:
    ...
    # This file lives at: <repo>/.team-os/scripts/requirements/_common.py
    p = Path(__file__).resolve()
    try:
        candidate = p.parents[3]
        if (candidate / ".team-os").exists() and (candidate / "AGENTS.md").exists():
            return candidate
    except Exception:
        pass
...
def add_template_app_to_syspath() -> None:
    ...
    """
    rr = repo_root()
    app_dir = rr / ".team-os" / "templates" / "runtime" / "orchestrator"
