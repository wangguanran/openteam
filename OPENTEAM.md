...
OpenTeam 以文件系统为“真相源”，结构见 `AGENTS.md`；静态资产在 repo 内，运行态与自管任务产物在 runtime root。
...
- 知识库：`~/.openteam/runtime/default/state/openteam/kb/**`
  - `sources/`：来源摘要（可追溯）
  - `roles/`：按角色沉淀的 Skill Cards
  - `platforms/`：按平台/子系统沉淀的 Skill Cards
- 角色长期记忆：`~/.openteam/runtime/default/state/openteam/memory/roles/<Role>/index.md`
- 台账（scope=openteam）：`~/.openteam/runtime/default/state/ledger/**`
- 任务日志（scope=openteam）：`~/.openteam/runtime/default/state/logs/tasks/<TASK_ID>/**`
- 项目台账/日志（scope=project:<id>）：`<WORKSPACE>/projects/<id>/state/ledger/**`、`<WORKSPACE>/projects/<id>/state/logs/**`
- 运行态状态（实例/Focus/项目/Workstream）：`~/.openteam/runtime/default/state/**`
...
台账文件 `~/.openteam/runtime/default/state/ledger/tasks/<TASK_ID>.yaml` 必须反映当前状态。
...
1. 来源摘要：`~/.openteam/runtime/default/state/openteam/kb/sources/`
2. Skill Card：`~/.openteam/runtime/default/state/openteam/kb/roles/` 或 `~/.openteam/runtime/default/state/openteam/kb/platforms/`
3. 角色记忆索引：`~/.openteam/runtime/default/state/openteam/memory/roles/<Role>/index.md`
