...
- **Deterministic Pipelines**：所有会改变真相源的动作必须由 `.team-os/scripts/pipelines/*.py` 执行。
...
- Crew Flow 定义：`.team-os/workflows/*.yaml`
- 知识库：`.team-os/kb/**`
  - `sources/`：来源摘要（可追溯）
...
新增 Crew Flow YAML 到 `.team-os/workflows/`（基于模板 `templates/workflow.yaml`），要求包含：
