...
- GitHub Projects v2 是 Team OS 的**主面板/视图层**（Table/Board/Roadmap），用于实时查看：
  ...
  - milestones（项目：`<WORKSPACE>/projects/<id>/state/plan/plan.yaml`；Team OS 自身：`docs/plans/teamos/plan.yaml`）
...
- Prompt 规范与编译产物：`specs/prompts/teamos/**`
...
- Team OS 自身需求主文档（scope=teamos）：`docs/product/teamos/requirements/**`
