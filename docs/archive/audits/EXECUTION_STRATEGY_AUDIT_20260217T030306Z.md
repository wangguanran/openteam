...
- DB integration (PostgreSQL) + migrations: FAIL  (requires OPENTEAM_DB_URL + migration runner)
...
- Always-on self-improve (auto enter on openteam run): FAIL  (expected: openteam auto starts daemon or control-plane schedules it; current design is daemon-only manual start)
