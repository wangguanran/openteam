...
## Hard Constraints
- No `.openteam/` runtime path in repo.
- Repo keeps static assets only.
- Runtime artifacts must be under `../openteam-runtime` (or `OPENTEAM_RUNTIME_ROOT` override).
...
- `RUNTIME_ROOT = <REPO_ROOT>/../openteam-runtime` by default
- Required runtime dirs:
  - `state/` (audit/logs/runs/openteam)
  - `workspace/`
  - `hub/`
  - `tmp/`
  - `cache/`
...
5. Fix remaining `.openteam` runtime writers that break purity during startup.
...
- `python3 scripts/pipelines/repo_purity_check.py --repo-root . --workspace-root ../openteam-runtime/workspace --json` => `ok=true`
...
- All execution logs and diagnostics: `../openteam-runtime/state/audit/`
- Final bootstrap verification report: `../openteam-runtime/state/audit/ONE_CLICK_BOOTSTRAP_VERIFY.md`
