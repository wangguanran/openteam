...
`doctor` 会在以下情况失败：repo 根存在 `.openteam/`，或存在 `runtime/workspace/hub/state/logs/ledger/tasks` 等运行态动态目录。

创建新任务：

```bash
cd openteam
./openteam task new --scope openteam --title "一句话需求标题" --workstreams "governance"
...
```bash
cd openteam
./openteam task close <TASK_ID> --scope openteam
```
...
```bash
cd openteam
./openteam task ship <TASK_ID> --scope openteam --summary "一句话变更摘要"
```

## 5. Runtime 部署与运维

Runtime 配置目录：`~/.openteam/runtime-config/default`

在新环境创建 runtime 配置目录（推荐，从 OpenTeam 模板生成；运行态 state/hub/cache/tmp 将进入 Docker named volumes）：
...
### 5.1 Control Plane（控制平面）与 openteam CLI
...
# 本地数据库：保持 OPENTEAM_DB_URL 为空；按需填写 POSTGRES_PASSWORD
# docker-compose 默认仍会启动本地 postgres；OPENTEAM_DB_URL 只决定 control-plane 连哪个库
make up
make ps
make logs
```
...
OpenTeam 强制 **Repo vs Workspace** 边界：

- `openteam/` git 仓库：只允许 scope=`openteam` 的真相源（OpenTeam 自身）。
- 任何 scope=`project:<id>` 的 requirements/冲突报告/ledger/logs/prompts/plan/项目 repo workdir：必须在 **Workspace**（不在 `openteam/` 目录树内）。

默认 Workspace 路径：

- `~/.openteam/workspace`（可通过 `OPENTEAM_WORKSPACE_ROOT` 或 CLI `--workspace-root` 覆盖）
...
- OpenTeam 自身（scope=`openteam`，允许在 repo 内）：`openteam/docs/product/openteam/requirements/**`
...
- `openteam workspace init` 幂等，可用于“修复”已存在项目目录的缺失结构（例如补齐 `repo/`、`state/prompts/MASTER_PROMPT.md`、`state/kb/`、`state/cluster/` 等）。

项目配置（Workspace-local，不入库）：

```bash
cd openteam
./openteam project config init --project demo
./openteam project config show --project demo
./openteam project config set --project demo --key panel.project_url --value "https://github.com/orgs/<org>/projects/<n>"
./openteam project config validate --project demo
```
...
```bash
cd openteam
./openteam project agents inject --project demo
```

说明：

- 注入区块使用标记替换：`<!-- OPENTEAM_MANUAL_START -->` / `<!-- OPENTEAM_MANUAL_END -->`（禁止手工编辑该区块）。
- `./openteam project config init|validate` 与 `./openteam req add|import|rebuild --scope project:<id>` 会自动触发注入（leader-only 写入，非 leader 为 plan-only）。

新增需求（两种方式等价，均遵守 Raw‑First；并会自动做可行性评估）：

```bash
cd openteam
./openteam req add "需求文本" --scope project:demo --workstream devops --priority P1
# OpenTeam 自身需求：
./openteam req add "改进 OpenTeam 的需求文本" --scope openteam --priority P2
```
...
```bash
cd openteam
./openteam req list --scope project:demo --show-conflicts
./openteam req conflicts --scope project:demo
./openteam req verify --scope project:demo
./openteam req rebuild --scope project:demo
```
...
```bash
cd openteam
./openteam req baseline show --scope project:demo
# 仅提案：baseline v2 需要理由，并进入 NEED_PM_DECISION（不会覆盖 v1）
./openteam req baseline set-v2 "新的 baseline 原文" --reason "为什么必须重述 baseline" --scope project:demo
```

注意：

- Expanded 文档（`requirements.yaml` / `REQUIREMENTS.md`）禁止手改；手改会在 `verify` 中被判定为 drift（并可通过 `rebuild` 恢复决定性渲染）。
- 并发安全：关键写入口（`./openteam req add|import|rebuild`、`./openteam prompt compile`、`./openteam task new|close`、以及 self-improve 的 requirements 更新）会先获取锁（repo lock + scope lock）。若命令返回 `LOCK_BUSY`，说明同 scope 正在被其他进程修改，按提示等待或重试。

### 5.3 Profiles（多实例）与 Workstream（多平台协作域）

Profiles 用于在同一台机器上管理多个 OpenTeam 实例（例如不同环境/不同项目组）：

- CLI 配置文件：`~/.openteam/config.toml`
- 添加/切换：

```bash
cd openteam
./openteam config add-profile dev http://127.0.0.1:8787 --default-project-id demo
./openteam config use dev
./openteam config show
```
...
```bash
cd openteam
./openteam status --project demo --workstream ai
./openteam agents --project demo --workstream devops
./openteam tasks --project demo --workstream web
```

### 5.4 面板（GitHub Projects）与同步（Panel Sync）

GitHub Projects v2 是 OpenTeam 的 **主面板/视图层**，用于：
...
- Projects 只是视图层，系统真相源仍是：
  - scope=`openteam`（OpenTeam 自身）：runtime state volume 内的 `ledger`、`openteam/docs/product/openteam/requirements/**`
...
字段（最小集合）由 sync 负责创建/复用（默认创建为 OpenTeam 自定义字段）：

- `OpenTeam Status`（单选：Todo/In Progress/In Review/Blocked/Done）
...
1. Table: Backlog
2. Board: Delivery（按 `OpenTeam Status` 分列）
...
然后把 `GITHUB_TOKEN` 写入本地 `~/.openteam/runtime-config/default/.env`（不入库）或通过环境变量注入。
...
> 安全闸门：Control Plane 默认禁用 GitHub 远程写入。执行真实同步前，需在 runtime 环境显式设置 `OPENTEAM_PANEL_GH_WRITE_ENABLED=1`。

```bash
cd openteam
./openteam panel sync --project demo --full
./openteam panel sync --project demo
...
OPENTEAM_PANEL_GH_SYNC_INTERVAL_SEC=60
```
...
- GitHub rate limit：降低 `OPENTEAM_PANEL_GH_SYNC_INTERVAL_SEC` 频率，或减少单次同步项数量（拆项目/分 workstream）
...
cd ~/.openteam/runtime-config/default
mkdir -p backups
# 备份 OpenTeam runtime DB（也可改成 pg_dumpall）
docker compose exec -T postgres pg_dump -U "$POSTGRES_USER" -d openteam > backups/openteam_$(date +%Y%m%d_%H%M%S).sql
```
...
```bash
cd ~/.openteam/runtime-config/default
cat backups/openteam_<timestamp>.sql | docker compose exec -T postgres psql -U "$POSTGRES_USER" -d openteam
```
...
- CLI：`cd openteam && ./openteam status`

## 7. 新任务怎么开始 (Genesis)

标准顺序（最小闭环）：

1. `./openteam task new`：生成台账与 `00~07` 日志骨架 + `metrics.jsonl`
...
1. 在 `07_retro.md` 写清“OpenTeam 的缺陷/改进点”
2. 启动 self-improve daemon（leader-only；默认只做 panel sync dry-run，不做 GitHub 写入）：

```bash
cd openteam
./openteam daemon start
./openteam daemon status
```

说明：

- Self-Improve 产物与 Raw Input 分离：
  - 提案：`.openteam/ledger/self_improve/<ts>-proposal.md`
  - Expanded 更新：通过系统通道更新 `docs/product/openteam/requirements/requirements.yaml`（不写入 `raw_inputs.jsonl`）

3. 需要立刻跑一轮（跳过 debounce）：

```bash
cd openteam
./openteam self-improve --force
```
...
- 审批由确定性 approvals 引擎落盘（优先写入 Postgres；无 DB 时写入 Workspace 审计文件），并可用 `./openteam approvals list` 查看
...
在 OpenTeam 仓库创建 issue（如已配置 remote）：

```bash
cd openteam
./openteam self-improve
```
...
```bash
openteam hub expose --bind-ip <private-ip> --allow-cidrs "10.0.0.0/24" --open-redis
```

Push hub config to node (contains secrets; approval required):

```bash
openteam hub push-config --host <node-ip> --user <user> --ssh-key ~/.ssh/id_ed25519
# or
printf '%s' "$SSH_PASSWORD" | openteam hub push-config --host <node-ip> --user <user> --password-stdin
```
