"""openteam_common.py -- Shared utilities for openteam.
...
def looks_like_openteam_repo(root: Path) -> bool:
    markers = (
...
def openteam_root() -> Path:
    ...
    2) relative to this file location (openteam_common.py lives at repo root)
    3) cwd walk-up
    """
    env = str(os.getenv("OPENTEAM_REPO_PATH") or "").strip()
    if env:
        p = Path(env).expanduser().resolve()
        if looks_like_openteam_repo(p):
            return p

    # This file lives at repo root
    here = Path(__file__).resolve().parent
    if looks_like_openteam_repo(here):
        return here

    # Walk up from cwd
    cwd = Path.cwd().resolve()
    for parent in [cwd, *cwd.parents]:
        if looks_like_openteam_repo(parent):
            return parent

    raise RuntimeError("Cannot locate openteam repo root (set OPENTEAM_REPO_PATH or run from within the repo)")
