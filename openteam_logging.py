"""openteam_logging.py -- Structured logging for openteam.
...
def _configure_once() -> None:
    ...
    root = logging.getLogger("openteam")
    level_name = os.getenv("OPENTEAM_LOG_LEVEL", "INFO").upper()
    root.setLevel(getattr(logging, level_name, logging.INFO))

    log_format = os.getenv("OPENTEAM_LOG_FORMAT", "json").lower()
...
def get_logger(name: str = "openteam") -> _StructuredLogger:
    """Get a structured logger. Name should be __name__ or a dotted path."""
    _configure_once()
    if not name.startswith("openteam"):
        name = f"openteam.{name}"
