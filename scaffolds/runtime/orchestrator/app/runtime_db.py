...
class SQLiteRuntimeDB:
    ...
    def _init(self) -> None:
        ...
        try:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_runtime_docs_ns_scope "
                "ON runtime_docs(namespace, scope_id, state, category, updated_at)"
            )
            conn.commit()
        finally:
            conn.close()
        ...
        with self._write_lock:
            ...
            try:
                ...
                if active and current.holder_instance_id and current.holder_instance_id != str(holder_instance_id):
                    ...
                    return None
                acquired_at = (
                    current.lease_acquired_at
                    if (active and current.holder_instance_id == str(holder_instance_id))
                    else now
                )
                conn.execute(
                    """
                    UPDATE task_leases
                    SET lease_scope = ?, project_id = ?, task_id = ?, holder_instance_id = ?,
                        holder_actor = ?, holder_meta_json = ?, lease_ttl_sec = ?,
    ...
    def get_panel_sync_summary(self, *, project_id: str, panel_type: str = "github_projects") -> dict[str, Any]:
        ...
        try:
            ...
            last_ok = conn.execute(
                "SELECT ts_end, mode, dry_run FROM panel_sync_runs "
                "WHERE panel_type = ? AND project_id = ? AND ok = 1 "
                "ORDER BY id DESC LIMIT 1",
                (str(panel_type), str(project_id)),
            ).fetchone()
            last_fail = conn.execute(
                "SELECT ts_end, mode, dry_run, error FROM panel_sync_runs "
                "WHERE panel_type = ? AND project_id = ? AND ok = 0 "
                "ORDER BY id DESC LIMIT 1",
                (str(panel_type), str(project_id)),
            ).fetchone()
            return {
    ...
    def list_runtime_state(self, *, namespace: str, prefix: str = "") -> list[dict[str, Any]]:
        ...
        try:
            if prefix:
                rows = conn.execute(
                    "SELECT state_key, value_json, updated_at FROM runtime_state "
...
class RuntimeDB:
    ...
    Single-node OpenTeam uses only the local sqlite runtime DB.
    """
