...
from app.domains.repo_improvement import proposal_runtime
from app.domains.repo_improvement import task_runtime


def _safe_target_from_context(context: Any) -> dict[str, Any]:
    project_id = proposal_runtime._safe_project_id(str(context.project_id or "teamos"))
    return proposal_runtime._resolve_target(
...
def _repo_context_outputs(*, target: dict[str, Any], workflow: Any, force: bool) -> dict[str, Any]:
    project_id = proposal_runtime._safe_project_id(str(target.get("project_id") or "teamos"))
    ...
    scan_repo_root = proposal_runtime._prepare_discovery_repo(source_repo_root=repo_root, target=target)
    repo_context = proposal_runtime.collect_repo_context(
        repo_root=repo_root,
    ...
    should_skip, skip_reason = proposal_runtime._should_skip(target_id=target_id, repo_root=repo_root, force=force)
    bug_scan_policy = {}
    if str(workflow.lane or "").strip().lower() == "bug":
        bug_scan_policy = proposal_runtime._bug_scan_policy(
            target_id=target_id,
            project_id=project_id,
    ...
    return {
        ...
        "repo_context": repo_context,
        "repo_context_blob": json.dumps(proposal_runtime._prompt_safe_repo_context(repo_context), ensure_ascii=False, indent=2),
...
@register_skill("repo.collect-context")
...
@register_skill("planning.materialize-findings")
def materialize_findings_skill(*, context: Any, inputs: dict[str, Any], state: dict[str, Any], spec: Any) -> dict[str, Any]:
    ...
    plan = proposal_runtime.UpgradePlan.model_validate(raw_plan)
    workflow = context.workflow
    project_id = proposal_runtime._safe_project_id(str(inputs.get("project_id") or context.project_id or "teamos"))
    target_id = str(inputs.get("target_id") or context.target_id or "").strip() or "teamos"
    repo_root = Path(str(inputs.get("repo_root") or proposal_runtime.team_os_root())).expanduser().resolve()
    ...
    filtered_findings = [finding for finding in list(plan.findings or []) if str(finding.lane or "").strip().lower() == str(workflow.lane or "").strip().lower()]
    filtered_plan = plan.model_copy(update={"findings": filtered_findings, "planned_version": plan.planned_version or proposal_runtime._planned_version(current_version, filtered_findings)})
    for finding in filtered_findings:
        work_items = list(finding.work_items or []) or proposal_runtime._default_work_items(repo_root=repo_root, finding=finding)
        if not workflow.uses_proposal:
            for work_item in work_items:
                records.append(
                    proposal_runtime._record_from_materialized_item(
                        target_id=target_id,
                        repo_root=repo_root,
                        repo_locator=repo_locator,
                        project_id=project_id,
            ...
            continue
        proposal = proposal_runtime._upsert_proposal(
            target_id=target_id,
            repo_root=repo_root,
            repo_locator=repo_locator,
            project_id=project_id,
        ...
        if workflow.requires_user_confirmation or bool(finding.requires_user_confirmation):
            proposal = proposal_runtime._ensure_proposal_discussion_issue(proposal)
        proposal_id = str(proposal.get("proposal_id") or "")
        status = str(proposal.get("status") or "").strip().upper()
        due = proposal_runtime._proposal_due(proposal)
        if workflow.should_materialize(status=status, due=due):
            ...
            if not dry_run:
                proposal_runtime._mark_proposal_materialized(proposal_id)
            continue
    ...
    if dry_run:
        ...
        panel_sync = proposal_runtime._sync_panel(db=context.db, project_id=project_id)
    report_fragment = {
        "summary": filtered_plan.summary,
        "current_version": current_version,
        "planned_version": filtered_plan.planned_version or proposal_runtime._planned_version(current_version, filtered_plan.findings),
...
@register_skill("github.claim-issue-discussion")
def claim_issue_discussion_skill(*, context: Any, inputs: dict[str, Any], state: dict[str, Any], spec: Any) -> dict[str, Any]:
    ...
    proposals = proposal_runtime.list_proposals(target_id=str(context.target_id or "").strip(), project_id=str(context.project_id or "").strip(), lane=lane)
    for proposal in proposals:
        status = str(proposal.get("status") or "").strip().upper()
        if status in ("REJECTED", "MATERIALIZED"):
            continue
        proposal = proposal_runtime._ensure_proposal_discussion_issue(proposal)
        issue_number = proposal_runtime._discussion_issue_number(proposal)
        ...
        new_comments = [comment for comment in comments if int(getattr(comment, "id", 0) or 0) > last_seen and proposal_runtime._comment_is_user_comment(comment)]
        if not new_comments:
            continue
        latest_comment_id = max(int(getattr(comment, "id", 0) or 0) for comment in new_comments)
        comments_text = "\n\n".join([str(getattr(comment, "body", "") or "").strip() for comment in new_comments if str(getattr(comment, "body", "") or "").strip()])
        return {
            "ok": True,
            "outputs": {
                "proposal": proposal,
                ...
                "explicit_action": proposal_runtime._proposal_action_from_comment_text(comments_text),
...
@register_skill("github.apply-issue-discussion")
def apply_issue_discussion_skill(*, context: Any, inputs: dict[str, Any], state: dict[str, Any], spec: Any) -> dict[str, Any]:
    ...
    response = proposal_runtime.ProposalDiscussionResponse.model_validate(inputs.get("response") or inputs.get("response_json") or {})
    ...
    if action in ("approve", "reject", "hold"):
        ...
        if str(response.module or "").strip():
            proposal = proposal_runtime._update_proposal_record(
    ...
    if issue_number > 0 and repo_locator:
        ...
        proposal = proposal_runtime._update_proposal_record(
            ...
            extra={
                ...
                "discussion_last_synced_at": proposal_runtime._utc_now_iso(),
...
@register_skill("delivery.run-task-pipeline")
def run_delivery_pipeline_skill(*, context: Any, inputs: dict[str, Any], state: dict[str, Any], spec: Any) -> dict[str, Any]:
    ...
    tasks = task_runtime.list_delivery_tasks(project_id=str(context.project_id or "").strip(), target_id=str(context.target_id or "").strip())
    ...
    for task in tasks:
        ...
        if not ledger_path.exists():
            continue
        doc = task_runtime._load_yaml(ledger_path)
        if str(task_runtime._task_lane(doc) or "bug").strip().lower() != lane:
            continue
    ...
    if not filtered:
        return {
            ...
            "outputs": {
                ...
                "processed": 0,
                "tasks": [],
                "summary": task_runtime.delivery_summary(project_id=str(context.project_id or "").strip(), target_id=str(context.target_id or "").strip()),
            },
    ...
    for task in filtered[:limit]:
        lease = task_runtime._claim_delivery_task_lease(db=context.db, actor=context.actor, task=task)
        if lease is None:
            out.append({"ok": True, "task_id": str(task.get("task_id") or ""), "skipped": True, "reason": "lease_held_by_other"})
            continue
        result = task_runtime._execute_delivery_candidate(
            db=context.db,
            actor=context.actor,
            ledger_path=Path(str(task.get("ledger_path") or "")).expanduser().resolve(),
            dry_run=bool(context.dry_run),
