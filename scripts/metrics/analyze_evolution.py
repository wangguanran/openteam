...
def analyze(summary: dict[str, Any], *, tasks_dir: Path) -> dict[str, Any]:
    ...
    # 3) If there are no events, the team workflow runtime has no signal.
    if int(summary.get("total_events") or 0) == 0:
        findings.append(
            {
                "kind": "OBSERVABILITY_GAP",
                "title": "Emit baseline telemetry events (TASK_CREATED, STATUS_CHANGED, TEAM_WORKFLOW_TRIGGERED)",
                "evidence": {"metrics_files": summary.get("metrics_files"), "total_events": summary.get("total_events")},
                "recommendation": "Add event writers to task creation/state transitions and the team workflow scheduler.",
            }
        )
