...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    req_script = repo / "scripts" / "pipelines" / "requirements_raw_first.py"
    ...
    if bool(getattr(args, "skip_self_improve", False)):
        ...
        si_script = repo / "scripts" / "pipelines" / "self_improve_daemon.py"
        r3 = _run(
            [
                sys.executable,
                str(si_script),
    ...
    if bool(getattr(args, "skip_db", False)):
        ...
        apv_script = repo / "scripts" / "pipelines" / "approvals.py"
        r6 = _run(
            [
                sys.executable,
                str(apv_script),
