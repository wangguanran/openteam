...
def cmd_workspace_migrate(args: argparse.Namespace) -> None:
    ...
    if apply:
        _approval_gate(
            ...
            summary="workspace migrate --from-repo --force (move legacy project artifacts out of team-os repo)",
...
def cmd_team_bug_scan_live(args: argparse.Namespace) -> None:
    ...
    cmd = [
        ...
        container,
        "python",
        "/team-os/scripts/runtime/team_bug_scan_live.py",
        "--team-id",
        team_id,
        "--target-id",
...
def cmd_openclaw_test(args: argparse.Namespace) -> None:
    base, _prof = _base_url(args)
    payload = {
        "message": str(getattr(args, "message", "") or "").strip() or "Team OS OpenClaw test message",
        "channel": str(getattr(args, "channel", "") or "").strip() or None,
        "target": str(getattr(args, "target", "") or "").strip() or None,
...
def cmd_daemon_start(args: argparse.Namespace) -> None:
    ...
    """
    raise RuntimeError("Legacy team daemon has been removed. Start the TeamOS runtime or run `teamos team run --team-id <team_id> --force`.")
...
def cmd_repo_create(args: argparse.Namespace) -> None:
    ...
    # Approval gate (records to DB when TEAMOS_DB_URL is set; otherwise local audit fallback).
    repo_root = _find_team_os_repo_root()
    if not repo_root:
        raise RuntimeError("Cannot find Team OS repo root. Set env TEAM_OS_REPO_PATH or run from within the repo.")
    _approval_gate(
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # when invoked as `teamos` (no subcommand) inside <workspace>/projects/<id>/repo,
    ...
    na.add_argument("--remote-env-path", default="~/.teamos/node.env", help="remote env path for hub config")
    ...
    hpc.add_argument("--remote-env-path", default="~/.teamos/node.env")
    ...
    dmig = db_sp.add_parser("migrate", help="Apply Postgres migrations (requires TEAMOS_DB_URL)")
    dmig.add_argument("--db-url", help="override TEAMOS_DB_URL")
    ...
    apv = sp.add_parser("approvals", help="High-risk approvals (DB-backed when TEAMOS_DB_URL is set)")
    ...
    dg = au_sp.add_parser("deterministic-gov", help="Generate deterministic governance audit report (scope=teamos)")
    ...
    ocs = sp.add_parser("openclaw-status", help="Show Team OS OpenClaw reporting status and detection")
    ocs.add_argument("--json", action="store_true", help="print raw JSON")
    ocs.set_defaults(fn=cmd_openclaw_status)

    occ = sp.add_parser("openclaw-config", help="Configure Team OS OpenClaw reporting route")
    ...
    oct.add_argument("--message", default="Team OS OpenClaw test message", help="message body")
