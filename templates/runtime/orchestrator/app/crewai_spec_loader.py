...
def specs_root() -> Path:
    return (Path(__file__).resolve().parent / "crewai_specs").resolve()


def _kind_root(kind: str) -> Path:
    return (specs_root() / str(kind or "").strip()).resolve()


def _team_root(team_id: str) -> Path:
    return (_kind_root("teams") / str(team_id or "").strip()).resolve()
...
def list_team_workflow_docs(team_id: str) -> tuple[dict[str, Any], ...]:
    ...
    if not wanted:
        return ()
    root = _team_root(wanted) / "workflows"
    if not root.exists():
        return ()
    docs: list[dict[str, Any]] = []
    for path in sorted(root.glob("*.yaml")):
...
def team_doc(team_id: str) -> dict[str, Any]:
    ...
    if not wanted:
        return {}
    nested = _load_yaml_file(_team_root(wanted) / "team.yaml")
    if nested:
        return nested
...
def team_workflow_doc(team_id: str, workflow_id: str) -> dict[str, Any]:
    ...
    if not wanted_team or not wanted_workflow:
        return {}
    return _load_yaml_file(_team_root(wanted_team) / "workflows" / f"{wanted_workflow}.yaml")
...
def team_stage_doc(team_id: str, stage_id: str) -> dict[str, Any]:
    ...
    if not wanted_team or not wanted_stage:
        return {}
    return _load_yaml_file(_team_root(wanted_team) / "stages" / f"{wanted_stage}.yaml")
