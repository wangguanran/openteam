...
from .requirements_store import AddReqOutcome, add_requirement_system_update
...
def _debounce_hours(repo_root: Path) -> int:
    pol = _read_yaml(repo_root / "policies" / "evolution_policy.yaml")
...
def scan_repo(repo_root: Path, *, api_routes: Optional[list[str]] = None) -> dict[str, Any]:
    ...
    wf_trunk = repo_root / "workflows" / "trunk.yaml"
    wf_plugins_dir = repo_root / "workflows" / "plugins"
    wf_missing = []
    if not wf_trunk.exists():
        wf_missing.append("workflows/trunk.yaml")
    if not wf_plugins_dir.exists():
        wf_missing.append("workflows/plugins/")
    else:
        for need in ["repo_understanding.yaml", "risk_gate.yaml"]:
            if not (wf_plugins_dir / need).exists():
                wf_missing.append(f"workflows/plugins/{need}")

    pol_missing = []
    if not (repo_root / "policies" / "evolution_policy.yaml").exists():
        pol_missing.append("policies/evolution_policy.yaml")

    # C) telemetry schema + task artifacts
    schema_missing = []
    if not (repo_root / "schemas" / "telemetry_event.schema.json").exists():
        schema_missing.append("schemas/telemetry_event.schema.json")
    ...
    *,
    ...
    for pr in props:
        ...
        out = add_requirement_raw_first(
            ...
            workstream_id=(pr.workstreams[0] if (pr.workstreams or []) else "general"),
