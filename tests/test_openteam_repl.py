...
class TeamosReplTests(unittest.TestCase):
    ...
    def test_project_repl_control_commands_do_not_write_raw(self) -> None:
        ...
        stdout = io.StringIO()
        with mock.patch("openteam_cli.project._base_url", return_value=("http://cp.local", {"name": "local"})), mock.patch(
            "openteam_cli.project._http_json", side_effect=fake_http
    ...
    def test_team_logs_defaults_to_latest_run(self) -> None:
        ...
        stdout = io.StringIO()
        with mock.patch("openteam_cli.team._base_url", return_value=("http://cp.local", {"name": "local"})), mock.patch(
            "openteam_cli.team._http_json", side_effect=fake_http
        ), contextlib.redirect_stdout(stdout):
    ...
    def test_team_watch_prints_sse_stream(self) -> None:
        ...
        stdout = io.StringIO()
        with mock.patch("openteam_cli.team._base_url", return_value=("http://cp.local", {"name": "local"})), mock.patch(
            "openteam_cli.team._resolve_team_watch_run_id", return_value="run-123"
        ), mock.patch("openteam_cli.team.urllib.request.urlopen", return_value=_FakeStream(sse)), contextlib.redirect_stdout(stdout):
