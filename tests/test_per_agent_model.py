...
class WorkflowAgentSpecFieldsTests(unittest.TestCase):
    def test_agent_spec_has_model_fields(self):
        spec = _workflow_registry().WorkflowAgentSpec(agent_id="t", role_id="T")
    ...
    def test_agent_spec_from_doc_parses_model(self):
        doc = {"agent_id": "scanner", "role_id": "Test-Manager", "model": "openai/gpt-4.1", "max_tokens": 16384}
        agent = _workflow_registry()._agent_spec_from_doc(doc)
        self.assertEqual(agent.model, "openai/gpt-4.1")
    ...
    def test_repo_review_agents_have_model_field(self):
        spec = _workflow_registry().workflow_spec("repo-review", team_id="repo-improvement", project_id="openteam")
...
class AgentLLMConfigTests(unittest.TestCase):
    def test_agent_model_overrides_global(self):
        ...
        agent = _workflow_registry().WorkflowAgentSpec(agent_id="t", role_id="T", model="google/gemini-2.5-flash")
        with mock.patch.dict(os.environ, {"OPENTEAM_LLM_MODEL": "openai/gpt-4.1"}):
    ...
    def test_empty_agent_model_inherits_global(self):
        from app.engines.llm_config import build_agent_llm_config
        agent = _workflow_registry().WorkflowAgentSpec(agent_id="t", role_id="T")
        with mock.patch.dict(os.environ, {"OPENTEAM_LLM_MODEL": "openai/gpt-4.1"}):
            cfg = build_agent_llm_config(agent_spec=agent)
    ...
    def test_agent_max_tokens_overrides(self):
        from app.engines.llm_config import build_agent_llm_config
        agent = _workflow_registry().WorkflowAgentSpec(agent_id="t", role_id="T", max_tokens=32768)
        cfg = build_agent_llm_config(agent_spec=agent)
        self.assertEqual(cfg.max_tokens, 32768)

    def test_agent_api_key_overrides(self):
        from app.engines.llm_config import build_agent_llm_config
        agent = _workflow_registry().WorkflowAgentSpec(agent_id="t", role_id="T", api_key="sk-agent-specific")
