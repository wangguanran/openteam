...
class RedisBusTests(unittest.TestCase):
    def test_describe_not_configured(self):
        with mock.patch.dict(os.environ, {"TEAMOS_REDIS_URL": ""}):
    ...
    def test_operations_graceful_when_not_configured(self):
        with mock.patch.dict(os.environ, {"TEAMOS_REDIS_URL": ""}):
            pub = redis_bus.publish_event("teamos.events", {"event_type": "TASK_NEW"})
            enq = redis_bus.enqueue("teamos.queue", {"task_id": "T-1"})
            deq = redis_bus.dequeue("teamos.queue", timeout=0)
    ...
    def test_missing_dependency_is_graceful(self):
        with mock.patch.dict(os.environ, {"TEAMOS_REDIS_URL": "redis://127.0.0.1:6379/0"}):
            with mock.patch.object(redis_bus, "_import_redis_module", side_effect=ImportError("redis not installed")):
                out = redis_bus.describe()
                pub = redis_bus.publish_event("teamos.events", {"event_type": "RUN_STARTED"})
